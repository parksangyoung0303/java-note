...
// 인터페이스 : 클래스가 구현해야만 하는 메서드들의 집합을 정의 해둔 것
// 인터페이스의 모든 변수는 public static final 형태임
// 인터페이스의 모든 메서드는 public abstract 형태임
// 인터페이스는 다중 구현이 가능함

// 인터페이스는 개발자간 상호 약속을 위한 수단으로 사용됨
// 기능, 상수들에 대한 약속을 정의함

// 접근제어자 interface 인터페이스명 {....}
interface AreaCalculation {
	
	// 변수는 반드시 public static final 이기 때문에 생략해도 됨
	public static final double PI = 3.14; // 밑에꺼랑 같음
//	double PI = 3.14; // 생략한 방법
	
	// 메소드는 반드시 public abstract 이기 때문에 생략을 해도 됨
	public abstract double getTriangleArea(double bottom, double height);
	double getRectangleArea(double bottom, double height);  // 밑에꺼랑 같이 생략한 방법
...
class Calculator implements Calculation /* AreaCalculation, NormalCalculation */ {
	...
	public double getTriangleArea(double bottom, double height) {
		...
		return triangleArea;
//		return  bottom * height / 2; 이렇게도 가능 밑에 것들도 같음
	}

	@Override
	public double getRectangleArea(double bottom, double height) {
		double rectangArea = bottom * height;
		return rectangArea;
	}
