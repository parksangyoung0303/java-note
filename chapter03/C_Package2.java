...
public class C_Package2 {
	...
	public static void main(String[] args) {
		...
		// Collection Framework에서 데이터 타입을 지정할 떈 제너릭을 사용하여 지정, 참조형 데이터 타입만 지정 가능
		List<Integer> list;	
		
		// ArrayList 구현 클래스 : 인덱스를 이용한 작업을 수행 할떄 유리함
		list = new ArrayList<>();
		// LinkedList 구현 클래스 : 데이터 추가, 삭제 작업을 수행 할 때 유리함
		list = new LinkedList<>();
		
		// add() : List에 요소를 추가
		// 추가할 요소는 제너릭으로 지정한 데이터 타입에 요소이어여 함
		list.add(10);
		// list.add("10");
		...
		// 기본형 데이터 타입의 변수로 리스트의 요소를 받을 땐 null 이 올수 있음을 주의
		Integer integer = list.get(0);
		System.out.println(integer);
		
		// set() : List의 특정 인덱스 요소를 변경
		// List 인덱스의 범위를 벗어난 인덱스를 지정할 수 없음
		list.set(1, -20); // 첫번째 자리 인덱스 번호, 두번째 자리 변경할 값
		System.out.println(list);
		
		// remove() : list의 특정 요소를 제거
		list.remove(0);
		System.out.println(list);
		...
		// Set: 데이터를 저장할 떄 중복을 허용하지 않고 순서가 존재하지 않는 데이터 구조 - 인덱스 사용하지 않음
		Set<String> set;
		...
		// List 와 Set 을 적절히 사용하면 List의 중복 제거 및 정렬을 손쉽게 수행할 수 있음
		list = new ArrayList<>();
		list.add(1);
		list.add(1);
		list.add(2);
		...
		// Map : key 와 value 한 쌍으로 이루어진 요소들의 집합
		// key는 중복 불가, value는 중복 가능
		Map<String, String> map;
		
		// HashMap 구현 클래스 : 검색이 빠르다
		map = new HashMap<>();
		...
		// remove(): Map에서 특정 key를 가지고 key와 value의
		...
		System.out.println(human2.get("Name")); // null 반환
		// 2. 문제점 : 타입의 안정성의 문제
