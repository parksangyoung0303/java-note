...
// 반드시 추상 클래스를 상속받은 하위 클래스에서 재정의(오버라이딩)하여 구현 해야함
// abstract 반환타입 추상메서드명(매개변수, .....);   - {}이건 안됨 

// abstract : 추상 클래스 혹은 추상 메서드를 정의하는 제어자
// abstract 가 붙은 클래스는 인스턴스 생성할 수 없음 / abstract 가 붙은 메서드는 구현부를 가질 수 없음
...
abstract class Occupation {
	...
	// 단, 기본생성자가 없는 상태에서 생성자를 정의 한다면, 하위 클래스에 생성자 작성을 강요함
	...
	abstract void work(); // 이게 선언부만 존재하고 구현부가 없는 방식
	
	// 추상클래스도 일반 메서드를 가질수 있음
	// 일반메서드는 오버라이딩을 강요하진 않음
	void sleep() {
...
public class H_AbstractClass {
	...
	public static void main(String[] args) {
		...
		TourGuide tourGuide = new TourGuide(10, 100, "대한민국");
		developer.work();
...
// private + final 이 붙은 메서드 : final 이 의미가 없음
// private + abstract 가 붙은 메서드 : 불가능 (상충) private 오버라이드 불가능 vs abstract 반드시 오버라이드 해야함
// final + abstract 가 붙은 클래스와 메서드 : 불가능 (상충) private 오버라이드 불가능 vs abstract 반드시 오버라이드 해야함
