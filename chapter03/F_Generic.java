...
// Test1, Test2, Test3 의 문제점
// 1. 코드 중복 발생 : 유지보수성 및 개발 속도를 낮춤
...
public class F_Generic {
	...
	// 제너릭 메서드 : 해당 메서드의 매개변수의 타입이나 반환 타입을 컴파일 시에 결정 할 때 사용
	// [제어자...] <매개타입> 반환타입 메서드명(...) {...}	
	...
	public static void main(String[] args) {
		...
		// Object 타입으로 지정하면 모든 타입을 받을 순 있지만, 사용시 타입을 지정함에 있어서 안전성 떨어짐
		// int score = (int) tests.get(0).score;
		
		// 제너릭으로 지정하면 컴파일 시 타입을 결정하여 원하는 데이터 타입으로 사용할 수 있음
		// 제너릭의 타입을 전달할 땐 반드시 참조형 데이터 타입이어야 함 ex) Integer, Boolean...
		Test5<Integer> instance1 = new Test5<>();
		// instance1.score = "우수";  매개타입 불일치 오류
		instance1.score = 100;
	
		// extends 키워드로 제한을 걸어둔 매개타입은 해당 클래스 혹은 하위 클래스만 제너릭으로 사용할 수 있음
		ExtendsClass<MyClass> instance2;
		ExtendsClass<SubClass> instance3;
		// ExtendsClass<SuperClass> instance4; // 상위클래스는 안됨
		// ExtendsClass<String> instance5;  // <> 사이에 관련없는 키워드는 안됨
		...
		// ? extends MyClass로 제한이 걸렸기 떄문에 MyClass 혹은 MyClass 하위 클래스만 전달 가능
		// extendsMethod(superList);
		extendsMethod(myList);
		extendsMethod(subList);
		
		// ? super MyClass로 제한이 걸렸기 떄문에 MyClass 혹은 MyClass 상위 클래스만 전달 가능
		superMethod(superList);
		superMethod(myList);
		// superMethod(subList);
		
		// 제너릭 메서드 사용시 - 메서드의 매개타 직접 지정하지 않고 매개변수의 타입을 따라감
		genericMethod(myList);
	}
	
	// ? : 제너릭의 와일드 카드로 모든 타입을 받을수 있도록 함, 와일드카드는 메서드의 매개변수 혹은 반환타입에 사용
