...
class ExampleClass1 {
	...
	// 필드의 데이터 앞에 static 키워드를 사용하여 지정
	// 인스턴스 생성 없이 사용 가능
	...
	// 클래스 메서드에서는 인스턴스 변수 사용 및 인스턴스 호출이 불가능
	// 인스턴스 변수 및 인스턴스 메서드를 사용하지 않는 기능에 주로 사용됨
	static int classMethod(int agr1) {
		// return agr1 * attribute1;
		return agr1 * classVariable;
	}
}

// 스마트폰 - 운영체제, 전화번호, 전원상태, 전원 상태 변경 작업, 정보를 보여주는 작업, 전화를 거는 작업
class SmartPhone1 {
	String os;
	String telNumber;
	boolean power;
...
// 삼각형 관련 수학 - 빗변 구하기, 둘레 구하기, 넓이 구하기
class TriangleMath {
	
	// SPEED_OF_LIGHT은 TriangleMath 클래스의 단일 책임 원칙에 위배됨
...
public class A_Class_Object {
	...
	static int fx1(int x) {
		...
		// 함수에서 return 작업이 수행되어야힘.
		// 단, void 반환 타입일때는 return 필수는 아님
		// void : 타입 없음
		return y;
	}
	
	// 함수의 목적
	// 1. 기능을 미리 정의해두고 사용할땐 호출하여 사용할 수 있도록 함 (코드 중복 제거, 유지 보수성 향상, 실제 구현 내용 알 필요 없음)
	...
	public static void main(String[] args) {
		...
		// 클래스 변수를 사용할 때 주의할 점
		// 클래스 변수는 모든 인스턴스에 영향을 미치기 때문에 클래스로 접근하길 권장함
		...
		galaxyS24.telNumber = "010-1111-2222";
